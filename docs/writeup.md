...
- **YS7704 (Vx.x): re-imported, in progress.** Hit the same interrupt-vector-table auto-analysis landmine as P0706 (bogus tiny functions decoded from raw vector-table bytes near the reset vector) but a tooling connection drop interrupted the fix mid-application - currently paused with the landmine identified but not yet resolved.
- YS7804 and YS8003 haven't had an analysis pass started yet.
...
- Part Two is itself mid-stream: YS7704's Ghidra analysis is paused partway through, YS7804/YS8003 haven't started, P1606's and P7805's FCC IDs are still unresolved, and the local MQTT broker's real-world network exposure still needs a physical hub to check.
